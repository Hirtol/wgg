...
use wgg_providers::SerdeWggCache;
...
pub async fn setup_cache(config: &Config) -> Option<SerdeWggCache> {
...
pub async fn teardown_cache(cache: SerdeWggCache, config: &Config) {
    let persist_path = full_path(&config.app.cache_dir);
...
async fn serialize_cache(cache: SerdeWggCache, config: &Config) -> anyhow::Result<()> {
...
async fn deserialize_cache(config: &Config) -> anyhow::Result<(SerdeWggCache, Option<SystemTime>)> {
